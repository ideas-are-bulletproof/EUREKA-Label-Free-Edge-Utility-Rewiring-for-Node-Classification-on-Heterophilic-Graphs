...
benchmark_others.py
====================
Faithful paper-exact implementations of seven graph rewiring / classification
baseline methods, evaluated on the same datasets and seeds as
eureka_benchmark.py.
...
Each method uses ONLY the classifier proposed in its paper:
  ...
  - DHGR      - graph rewiring via the official DHGR repository
                Bi et al., "Make Heterophilic Graphs Better Fit GNN"
                (requires the official DHGR repo on disk, see README)
  - FoSR      - first-order spectral rewiring, followed by a GCN
                Karhadkar, Banerjee, Montufar
  - ComFy     - Louvain community + feature-similarity guided rewiring
                Rubio-Madrigal, Jamadandi, Burkholz, ICLR 2025, Algorithm 6

Shares with eureka_benchmark.py:
  - Identical dataset loading functions (copy-pasted, no import dependency)
  - Identical seeds list [0, 1, 2] by default
  - Identical train/val/test masks (same loading logic, same fixed splits)

Output:
  others_benchmark_results/results_faithful.jsonl   - per-seed records
  others_benchmark_results/summary_faithful.csv     - mean +/- std per (dataset, method)
